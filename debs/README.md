...
这里保存当前签名目录和七个可安装的软件包：Launcher 0.2、Store 0.3、Reader 0.4（PDF、未加密 Kindle 格式与 OPDS）、Gamer 0.2、MyAI 0.3、微信安装适配程序 0.1.2（修复已安装时的启动提示）、Copilot 0.2.0（全目录统一写入与本地缓存管理）。Reader、Gamer、MyAI 均包含实际程序；Gamer 的 ROM 与核心由用户提供。微信适配包包含完整安装/启动代码，腾讯原客户端由设备直接从官网下载，未在 GitHub 镜像。
